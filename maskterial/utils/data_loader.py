...
class ContrastDataloader:
    ...
    def load_contrasts_from_directory(
        ...
        for idx, mask_name in enumerate(mask_names):
            ...
            # i.e. the background of any of the channels is black
            if np.any(background_color < 1):
                print(f"\nInvalid background color {background_color} for image {mask_name}, skipping...")
                skipped_invalid_background += 1
                continue
            ...
            if background_pixels.shape[0] > 0:
                sampled_indices = np.random.choice(
                    ...
                    min(10000, background_pixels.shape[0]), 
                    replace=True
                )
                X.extend(background_pixels[sampled_indices])
                y.extend([0] * len(sampled_indices))
        ...
        print(f"  - Skipped (invalid background): {skipped_invalid_background}")
        print(f"Total samples collected: {len(X)}")
        if len(X) == 0:
            print("\n⚠️  ERROR: No valid samples were collected!")
            print("Possible reasons:")
            print("  1. All images have invalid (black/zero) backgrounds")
            print("     → Check if images need flatfield correction")
            print("  2. Semantic masks are empty or contain only background (class 0)")
            print("     → Make sure you've run the Class Annotator and saved semantic masks")
            print("  3. Image files don't match mask files")
            print("     → Check that image and mask filenames correspond")
            print(f"\nPaths checked:")
    ...
    def _apply_Nearest_Neighbors(self):
        ...
        if len(self.X_train) == 0:
            raise ValueError(
                ...
                "This usually means:\n"
                "1. No valid images were found (check image paths)\n"
                "2. All images had invalid backgrounds (all black/zero)\n"
                "3. Semantic masks are empty or missing class labels\n"
                "Please check your semantic_masks directory and ensure images have valid backgrounds."
            )
