...
def fit_set(
    ...
    **kwargs,
    ...
    Parameters:
    -----------
    data (np.ndarray): Input data of shape [n_samples, 3], typically representing color values (e.g., BGR).
    num_components (int): Number of desired GMM components.
    cov_type (str): covariance type for the GMM: "full", "tied", or "diag".
    num_additional_noise_comp (int, optional): Extra components to be added temporarily to model noise (these will be removed afterward).
    sample_size (int, optional):  Number of data points to use for GMM training. If None, all data is used.
    used_channels (str, optional): String of channels to use from "BGR" (e.g., "BG", "GR", etc.).
    initial_means (np.ndarray, optional): Optional initial means for the GMM.
    **kwargs : Additional keyword arguments passed to `GaussianMixture`.

    Returns:
    --------
    all_means_gauss (np.ndarray): Means of the final GMM components, shape [n_components, 3].
    all_covariances_gauss (np.ndarray): Covariance matrices of the final GMM components, shape [n_components, 3, 3].
    all_weights_gauss (np.ndarray): Mixture weights (probabilities) of each GMM component.
    sampled_data (np.ndarray): Data points actually used for fitting.
    predicted_labels (np.ndarray): Predicted component labels for the sampled data.
    """
